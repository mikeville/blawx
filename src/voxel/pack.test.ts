...
test('16³ dense layer packs cleanly (grid-size independence)', () => {
  const voxels: Voxel[] = [];
  for (let x = 0; x < 16; x++) {
    for (let z = 0; z < 16; z++) {
      voxels.push(v(x, z, 'blue'));
    }
  }
  const out = packLayer(voxels);
  // 16×16 all same color → 64 clean 2×2 bricks
  assert.equal(out.length, 64);
  assert.ok(out.every(b => b.w === 2 && b.d === 2));
});
